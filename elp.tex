...
\begin{algorithm}
  \KwIn{An \elp TBox $\tb = \{a_1, \ldots, a_n\}$ over $\nc$ and $\nr$, and $A, B \in \nc$ as well as a \enquote{support set} (of axioms) for $A \subsume_{\tb} B$.}
  \KwOut{minimal subset $\mathcal{S} \subseteq \tb$ s.t.~$A \subsume_{\tb \cup \mathcal{S}} B$ holds.}
  \If{$|\tb| = 1$}{
    \Return{$\tb$}
  }
