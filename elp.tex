...
Since all \elp TBoxes are consistent by definition, satisfiability is trivial. Instead, we are generally interested in deciding the reasoning problem of \emph{concept subsumption}.
...
\begin{algorithm}
  ...
  \While{there is an $i$ s.t. $1 \leq i \leq 5$, $a_i \in \tb$ and $P_i \subseteq \mathcal{A}$ \label{alg:pin:loop:begin}\tcp{see Table \ref{tbl:rules}}}{
      $\phi = lab(a_i) \wedge lab(P_i)$ \\
      \uIf{$q_i \not \in \mathcal{A}$}{
          \tcp{$a_i$ is the only known explanation of $a_i$.}
        $\mathcal{A} := \mathcal{A} \cup \{ q_i \}$\\
        $lab(q_i) := \phi$\\
      }
      \Else{\tcp{$q_i$ can already be explained, $a_i$ might be a new explanation!}
        $\psi = lab(q_i)$ \\
        \If{$\psi \vee \phi \not \equiv \psi$}{
...
\begin{algorithm}
  ...
  $L := \{ a_i \in \tb \mid 1 \leq i \leq \nicefrac{n}{2} \}$ \label{alg:logone:split1}\\
  $R := \{ a_i \in \tb \mid \nicefrac{n}{2} + 1 \leq i \leq n \}$ \label{alg:logone:split2}\\
